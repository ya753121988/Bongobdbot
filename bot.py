import asyncio
import os
import requests
import json
import logging
from quart import Quart, render_template_string, jsonify, request
from quart_cors import cors
from aiogram import Bot, Dispatcher, types
from aiogram.contrib.fsm_storage.memory import MemoryStorage
from aiogram.dispatcher import FSMContext
from aiogram.dispatcher.filters.state import State, StatesGroup
from motor.motor_asyncio import AsyncIOMotorClient
from datetime import datetime

# ================= CONFIGURATION (YOUR DATA) =================
API_TOKEN = '8162462190:AAFqdr69Et6tQ4_CvW3OjNjniu8yz056TCM'
MONGO_URL = 'mongodb+srv://roxiw19528:roxiw19528@cluster0.vl508y4.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0'
ADMIN_ID = 7120801813
APP_URL = "https://bongobdbot.onrender.com"

# ================= DB & BOT SETUP =================
client = AsyncIOMotorClient(MONGO_URL)
db = client['bongobd_master_db']
bot = Bot(token=API_TOKEN)
dp = Dispatcher(bot, storage=MemoryStorage())

class PostState(StatesGroup):
    title = State()
    photo = State()
    content = State()

# ================= MINI APP UI (ULTRA DESIGN) =================
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="bn">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    <title id="web-title">BongoBD Viral</title>
    <script src="https://telegram.org/js/telegram-web-app.js"></script>
    <style>
        :root { --pink: #ff006e; --bg: #000; --card: #121212; --text: #fff; --gray: #888; }
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; outline: none; }
        body { background: var(--bg); color: var(--text); font-family: 'Segoe UI', Roboto, sans-serif; margin: 0; padding: 0; overflow-x: hidden; }
        
        /* Notice Bar */
        #notice-bar { background: var(--pink); color: #fff; padding: 12px; text-align: center; font-size: 14px; font-weight: bold; display: none; position: sticky; top: 0; z-index: 2000; box-shadow: 0 2px 10px rgba(255,0,110,0.5); }
        
        /* Header */
        header { padding: 20px; text-align: center; border-bottom: 1px solid #222; background: rgba(0,0,0,0.9); backdrop-filter: blur(10px); }
        .logo { color: var(--pink); font-size: 26px; font-weight: 900; letter-spacing: 1px; text-transform: uppercase; }

        .container { padding: 15px; max-width: 1200px; margin: 0 auto; min-height: 100vh; }
        .section-title { font-size: 18px; margin: 25px 0 15px; display: flex; align-items: center; gap: 10px; font-weight: bold; }
        .section-title::before { content: ''; width: 4px; height: 22px; background: var(--pink); border-radius: 2px; }

        /* Slider */
        .slider { display: flex; overflow-x: auto; gap: 15px; padding-bottom: 15px; scrollbar-width: none; }
        .slider::-webkit-scrollbar { display: none; }
        .slide-item { min-width: 280px; height: 160px; background: var(--card); border-radius: 15px; overflow: hidden; position: relative; border: 1px solid #333; flex-shrink: 0; }
        .slide-item img { width: 100%; height: 100%; object-fit: cover; opacity: 0.5; }
        .slide-label { position: absolute; bottom: 12px; left: 12px; font-size: 14px; font-weight: bold; text-shadow: 0 2px 4px #000; }

        /* Responsive Grid (Desktop & Mobile) */
        .grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; }
        @media (min-width: 768px) { .grid { grid-template-columns: repeat(4, 1fr); } }
        @media (min-width: 1024px) { .grid { grid-template-columns: repeat(5, 1fr); } }

        .card { background: var(--card); border-radius: 12px; overflow: hidden; border: 1px solid #222; transition: 0.3s; position: relative; }
        .card:hover { border-color: var(--pink); transform: translateY(-5px); }
        .card-img { width: 100%; height: 160px; object-fit: cover; }
        .card-body { padding: 12px; }
        .card-title { font-size: 14px; font-weight: 600; height: 40px; overflow: hidden; margin-bottom: 12px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; }
        .unlock-btn { background: var(--pink); color: #fff; border: none; width: 100%; padding: 12px; border-radius: 8px; font-weight: bold; cursor: pointer; transition: 0.2s; font-size: 13px; }
        .unlock-btn:active { transform: scale(0.95); opacity: 0.8; }

        /* Pagination */
        .pagination { display: flex; justify-content: center; align-items: center; gap: 20px; padding: 40px 0 120px; }
        .pg-btn { background: #1a1a1a; border: 1px solid var(--pink); color: #fff; padding: 12px 25px; border-radius: 10px; cursor: pointer; font-weight: bold; transition: 0.3s; }
        .pg-btn:disabled { border-color: #333; color: #555; cursor: not-allowed; }
        .pg-btn:hover:not(:disabled) { background: var(--pink); }

        /* Profile Page */
        .profile-hero { background: linear-gradient(180deg, var(--pink) 0%, #000 100%); padding: 60px 20px; text-align: center; }
        .avatar { width: 100px; height: 100px; background: #fff; border-radius: 50%; margin: 0 auto 15px; display: flex; align-items: center; justify-content: center; font-size: 40px; color: #000; font-weight: 900; border: 5px solid rgba(255,255,255,0.2); }
        .stats-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-top: 35px; }
        .stat-card { background: rgba(255,255,255,0.06); padding: 20px 10px; border-radius: 18px; border: 1px solid #222; }
        .stat-val { font-size: 22px; font-weight: bold; color: var(--pink); }
        .stat-lab { font-size: 10px; color: #aaa; text-transform: uppercase; margin-top: 6px; letter-spacing: 1px; }

        /* Navigation Menu */
        .navbar { position: fixed; bottom: 0; width: 100%; background: rgba(10,10,10,0.95); backdrop-filter: blur(20px); display: flex; justify-content: space-around; padding: 15px 0; border-top: 1px solid #222; z-index: 1000; }
        .nav-link { color: #666; text-decoration: none; font-size: 11px; text-align: center; flex: 1; transition: 0.3s; cursor: pointer; }
        .nav-link.active { color: var(--pink); font-weight: bold; }
        .nav-icon { font-size: 24px; margin-bottom: 5px; display: block; }

        .hidden { display: none; }
        #loader { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); color: var(--pink); font-size: 18px; font-weight: bold; }
    </style>
</head>
<body>
    <div id="notice-bar"></div>
    <header><div class="logo" id="app-name-top">BONGOBD</div></header>

    <!-- Pages Container -->
    <div id="loader">লোড হচ্ছে...</div>

    <!-- Home Page -->
    <div id="page-home" class="container hidden">
        <div class="section-title">পপুলার ভিডিও স্লাইডার</div>
        <div class="slider" id="slider-content"></div>

        <div class="section-title">নতুন সব ভিডিও</div>
        <div class="grid" id="video-grid"></div>

        <div class="pagination">
            <button class="pg-btn" id="btn-prev" onclick="changePage(-1)">← Previous</button>
            <span id="page-num" style="font-weight:bold; color:var(--pink);">Page 1 of 1</span>
            <button class="pg-btn" id="btn-next" onclick="changePage(1)">Next →</button>
        </div>
    </div>

    <!-- Profile Page -->
    <div id="page-profile" class="hidden">
        <div class="profile-hero">
            <div class="avatar" id="user-initial">?</div>
            <h2 id="user-full-name">Guest User</h2>
            <div id="user-handle" style="color:rgba(255,255,255,0.7); font-size:13px;">@username</div>
            <div class="stats-grid">
                <div class="stat-card"><div class="stat-val" id="stat-liked">0</div><div class="stat-lab">Liked</div></div>
                <div class="stat-card"><div class="stat-val" id="stat-today">0</div><div class="stat-lab">Today Ads</div></div>
                <div class="stat-card"><div class="stat-val">30</div><div class="stat-lab">Daily Limit</div></div>
            </div>
        </div>
    </div>

    <!-- Links / Tutorial Page (Placeholder) -->
    <div id="page-links" class="container hidden" style="text-align:center; padding-top:100px;">
        <h3 class="pink">তথ্য ও লিংক</h3>
        <p style="color:#888;">এখানে গুরুত্বপূর্ণ লিংক এবং টিউটোরিয়াল দেখানো হবে।</p>
    </div>

    <!-- Global Navigation -->
    <div class="navbar">
        <div class="nav-link active" onclick="navigate('home', this)"><span class="nav-icon">🏠</span>হোম</div>
        <div class="nav-link" onclick="navigate('home', this)"><span class="nav-icon">❤️</span>পছন্দ</div>
        <div class="nav-link" onclick="navigate('links', this)"><span class="nav-icon">ℹ️</span>টিউটোরিয়াল</div>
        <div class="nav-link" onclick="navigate('links', this)"><span class="nav-icon">🔗</span>লিংক</div>
        <div class="nav-link" onclick="navigate('profile', this)"><span class="nav-icon">👤</span>প্রোফাইল</div>
    </div>

    <script>
        let tg = window.Telegram.WebApp;
        tg.expand();
        let user = tg.initDataUnsafe.user || {id: 0, first_name: 'Guest', username: 'user'};
        let currentPage = 1;
        let globalAdStep = 0;

        async function initApp() {
            try {
                const setReq = await fetch('/api/settings');
                const set = await setReq.json();
                
                document.getElementById('web-title').innerText = set.name;
                document.getElementById('app-name-top').innerText = set.name;
                
                if(set.notice) {
                    const nb = document.getElementById('notice-bar');
                    nb.innerText = set.notice;
                    nb.style.display = 'block';
                }
                
                await loadSlider(set.slider_count || 5);
                await loadPosts(1);
                
                document.getElementById('loader').style.display = 'none';
                document.getElementById('page-home').classList.remove('hidden');
            } catch(e) {
                document.getElementById('loader').innerText = "সার্ভার এরর! পুনরায় চেষ্টা করুন।";
            }
        }

        async function loadSlider(count) {
            const res = await fetch(`/api/slider?count=${count}`);
            const data = await res.json();
            const slider = document.getElementById('slider-content');
            slider.innerHTML = '';
            data.forEach(p => {
                slider.innerHTML += `
                    <div class="slide-item">
                        <img src="${p.photo_url}">
                        <div class="slide-label">${p.title}</div>
                    </div>`;
            });
        }

        async function loadPosts(page) {
            currentPage = page;
            const res = await fetch(`/api/posts?page=${page}`);
            const data = await res.json();
            const grid = document.getElementById('video-grid');
            grid.innerHTML = '';
            
            data.posts.forEach(p => {
                grid.innerHTML += `
                    <div class="card">
                        <img src="${p.photo_url}" class="card-img">
                        <div class="card-body">
                            <div class="card-title">${p.title}</div>
                            <div id="step-info-${p._id}" style="font-size:11px;color:var(--pink);margin-bottom:8px;">ADS STEP: 0/${data.target_steps}</div>
                            <button class="unlock-btn" onclick="processUnlock('${p._id}', ${data.target_steps})">🎬 UNLOCK VIDEO</button>
                        </div>
                    </div>`;
            });
            
            document.getElementById('page-num').innerText = `Page ${page} of ${data.total_pages}`;
            document.getElementById('btn-prev').disabled = (page === 1);
            document.getElementById('btn-next').disabled = (page >= data.total_pages);
        }

        async function processUnlock(id, target) {
            const sdksRes = await fetch('/api/sdks');
            const sdks = await sdksRes.json();
            
            if(globalAdStep < target) {
                if(sdks.length > 0) {
                    const randomAd = sdks[Math.floor(Math.random() * sdks.length)].link;
                    window.open(randomAd, '_blank');
                }
                globalAdStep++;
                document.getElementById('step-info-'+id).innerText = `ADS STEP: ${globalAdStep}/${target}`;
                
                // Update click stats in background
                fetch('/api/ad-click', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({user_id: user.id})
                });
            } else {
                alert("ভিডিও সফলভাবে আনলক হয়েছে! মুভি লোড হচ্ছে...");
                globalAdStep = 0;
                // আপনি এখানে ভিডিও লিংকে রিডাইরেক্ট করতে পারেন
            }
        }

        function navigate(pageId, el) {
            document.querySelectorAll('.nav-link').forEach(l => l.classList.remove('active'));
            el.classList.add('active');
            
            document.querySelectorAll('.container, #page-profile, #page-links').forEach(p => p.classList.add('hidden'));
            
            const target = document.getElementById('page-' + pageId);
            if(target) target.classList.remove('hidden');
            
            if(pageId === 'profile') fetchProfileData();
            window.scrollTo(0, 0);
        }

        async function fetchProfileData() {
            const res = await fetch(`/api/user/${user.id}`);
            const data = await res.json();
            document.getElementById('user-full-name').innerText = user.first_name + ' ' + (user.last_name || '');
            document.getElementById('user-handle').innerText = '@' + (user.username || 'user');
            document.getElementById('user-initial').innerText = user.first_name[0].toUpperCase();
            document.getElementById('stat-liked').innerText = data.liked || 0;
            document.getElementById('stat-today').innerText = data.ads_today || 0;
        }

        function changePage(delta) {
            loadPosts(currentPage + delta);
            window.scrollTo(0, 0);
        }

        initApp();
    </script>
</body>
</html>
"""

# ================= API ENDPOINTS (QUART ASYNC) =================
app = Quart(__name__)
app = cors(app)

@app.route('/')
async def index():
    return render_template_string(HTML_TEMPLATE)

@app.route('/api/settings')
async def get_settings():
    s = await db.settings.find_one({"id": "config"}) or {"name": "BongoBD", "notice": "", "steps": 3, "slider_count": 6}
    return jsonify(s)

@app.route('/api/posts')
async def get_posts():
    page = int(request.args.get('page', 1))
    limit = 20
    total = await db.posts.count_documents({})
    posts = await db.posts.find().sort('_id', -1).skip((page-1)*limit).limit(limit).to_list(limit)
    for p in posts: p['_id'] = str(p['_id'])
    
    set_db = await db.settings.find_one({"id": "config"}) or {"steps": 3}
    return jsonify({
        "posts": posts,
        "total_pages": (total // limit) + (1 if total % limit > 0 else 0),
        "target_steps": set_db.get("steps", 3)
    })

@app.route('/api/slider')
async def get_slider():
    count = int(request.args.get('count', 6))
    posts = await db.posts.find().sort('_id', -1).limit(count).to_list(count)
    for p in posts: p['_id'] = str(p['_id'])
    return jsonify(posts)

@app.route('/api/user/<int:uid>')
async def get_user_data(uid):
    u = await db.users.find_one({"user_id": uid}) or {"ads_today": 0, "liked": 0}
    return jsonify(u)

@app.route('/api/ad-click', methods=['POST'])
async def register_click():
    data = await request.json
    uid = data.get('user_id')
    await db.users.update_one({"user_id": uid}, {"$inc": {"ads_today": 1}}, upsert=True)
    return jsonify({"success": True})

@app.route('/api/sdks')
async def get_ads():
    return jsonify(await db.sdks.find().to_list(20))

# ================= TELEGRAM BOT LOGIC =================

@dp.message_handler(commands=['start'])
async def cmd_start(m: types.Message):
    # Register user
    await db.users.update_one({"user_id": m.from_user.id}, {"$set": {"name": m.from_user.full_name}}, upsert=True)
    
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(types.InlineKeyboardButton("🎬 ওপেন মিনি অ্যাপ", web_app=types.WebAppInfo(url=APP_URL)))
    
    # Add dynamic buttons if any
    channels = await db.channels.find().to_list(10)
    for c in channels:
        markup.add(types.InlineKeyboardButton(c['name'], url=c['link']))

    welcome = f"✨ **স্বাগতম {m.from_user.first_name}**\n\nআমাদের ভিডিও সার্ভিস ব্যবহার করার জন্য নিচের বাটনে ক্লিক করে অ্যাপটি ওপেন করুন 👇"
    await m.answer_photo(photo="https://telegra.ph/file/a8677c3e1c66288828b80.jpg", caption=welcome, reply_markup=markup, parse_mode="Markdown")

# Admin Management Commands
@dp.message_handler(commands=['post'], user_id=ADMIN_ID)
async def admin_post(m: types.Message):
    await m.answer("📝 ভিডিও টাইটেল দিন:"); await PostState.title.set()

@dp.message_handler(state=PostState.title)
async def admin_title(m: types.Message, state: FSMContext):
    await state.update_data(title=m.text); await m.answer("📸 ভিডিওর জন্য একটি ফটো পাঠান:"); await PostState.photo.set()

@dp.message_handler(content_types=['photo'], state=PostState.photo)
async def admin_photo(m: types.Message, state: FSMContext):
    file = await bot.get_file(m.photo[-1].file_id)
    url = f"https://api.telegram.org/file/bot{API_TOKEN}/{file.file_path}"
    await state.update_data(photo=url); await m.answer("🔗 ভিডিও লিংক বা ফাইল পাঠান:"); await PostState.content.set()

@dp.message_handler(state=PostState.content, content_types=['any'])
async def admin_done(m: types.Message, state: FSMContext):
    data = await state.get_data()
    val = m.text or m.video.file_id or m.document.file_id
    await db.posts.insert_one({"title": data['title'], "photo_url": data['photo'], "content": val, "created_at": datetime.now()})
    await m.answer("✅ ভিডিও সফলভাবে পোস্ট করা হয়েছে!"); await state.finish()

@dp.message_handler(commands=['name'], user_id=ADMIN_ID)
async def admin_name(m: types.Message):
    name = m.get_args()
    if not name: return await m.answer("ব্যবহার: /name BongoBD")
    await db.settings.update_one({"id": "config"}, {"$set": {"name": name}}, upsert=True)
    await m.answer(f"✅ অ্যাপের নাম পরিবর্তন হয়ে '{name}' হয়েছে।")

@dp.message_handler(commands=['notice'], user_id=ADMIN_ID)
async def admin_notice(m: types.Message):
    txt = m.get_args()
    await db.settings.update_one({"id": "config"}, {"$set": {"notice": txt}}, upsert=True)
    await m.answer("✅ নোটিশ বার আপডেট হয়েছে।")

@dp.message_handler(commands=['delnotice'], user_id=ADMIN_ID)
async def admin_del_notice(m: types.Message):
    await db.settings.update_one({"id": "config"}, {"$set": {"notice": ""}}, upsert=True)
    await m.answer("🗑 নোটিশ ডিলিট করা হয়েছে।")

@dp.message_handler(commands=['step'], user_id=ADMIN_ID)
async def admin_step(m: types.Message):
    try:
        val = int(m.get_args())
        await db.settings.update_one({"id": "config"}, {"$set": {"steps": val}}, upsert=True)
        await m.answer(f"⚙️ অ্যাড আনলক স্টেপ {val} এ সেট করা হয়েছে।")
    except: await m.answer("ব্যবহার: /step 3")

@dp.message_handler(commands=['sdk'], user_id=ADMIN_ID)
async def admin_sdk(m: types.Message):
    args = m.get_args().split()
    if len(args) < 2: return await m.answer("ব্যবহার: /sdk 1 https://ad-link.com")
    await db.sdks.update_one({"id": args[0]}, {"$set": {"link": args[1]}}, upsert=True)
    await m.answer(f"✅ SDK {args[0]} আপডেট করা হয়েছে।")

@dp.message_handler(commands=['addchannel'], user_id=ADMIN_ID)
async def admin_add_ch(m: types.Message):
    args = m.get_args().split(maxsplit=1)
    await db.channels.update_one({"name": args[0]}, {"$set": {"link": args[1]}}, upsert=True)
    await m.answer(f"✅ বাটন '{args[0]}' এড হয়েছে।")

@dp.message_handler(commands=['deleteall'], user_id=ADMIN_ID)
async def admin_clear(m: types.Message):
    await db.posts.delete_many({}); await db.users.delete_many({}); await m.answer("💥 সব ডাটা মুছে ফেলা হয়েছে!")

# ================= RUNNER LOGIC =================
@app.before_serving
async def startup():
    # Setup Webhook automatically
    webhook_url = f"https://api.telegram.org/bot{API_TOKEN}/setWebhook?url={APP_URL}/tg-webhook"
    requests.get(webhook_url)
    # Start bot polling as fallback if needed, but we use Webhook
    asyncio.create_task(dp.start_polling())

@app.route('/tg-webhook', methods=['POST'])
async def telegram_webhook():
    update_data = await request.json
    update = types.Update.to_object(update_data)
    Dispatcher.set_current(dp)
    Bot.set_current(bot)
    await dp.process_update(update)
    return "ok", 200

if __name__ == '__main__':
    # রান কমান্ড: python bot.py
    port = int(os.environ.get('PORT', 5000))
    app.run(host='0.0.0.0', port=port)
